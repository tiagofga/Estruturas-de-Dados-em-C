...
Repositório educacional com implementações de estruturas de dados e algoritmos em C, cobrindo estruturas fundamentais e avançadas com exemplos executáveis, testes automatizados, documentação por módulo e exercícios práticos.
...
| Módulo | Build | Testes | Sanitizer | Documentação PT | Documentação EN |
|--------|-------|--------|-----------|-----------------|-----------------|
| Lista/Fila/Pilha | CI | CI | — | Sim | Sim |
| Heap | CI | CI | CI | Sim | Sim |
| TabelaHash | CI | CI | CI | Sim | Sim |
| ListaEncadeada / ListaDuplamenteEncadeada | CI | CI | CI | Sim | Sim |
| Árvore / AVL | CI | CI | AVL | Sim | Sim |
| Grafo / GrafoListaAdjacencia | CI | CI | GrafoListaAdjacencia | Sim | Sim |
| AlgoritmosGrafos | CI | CI | CI | Sim | Sim |
| Busca / Ordenação | CI | CI | — | Sim | Sim |
...
Os testes do projeto estão organizados nas pastas `tests/` de cada módulo implementado. A CI compila e testa todos os módulos com implementação em C e executa sanitizers nos módulos novos e avançados.
