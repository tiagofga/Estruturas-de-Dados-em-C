...
static void test_criar_e_destruir(void) {
    TabelaHash *tabela = tabela_hash_criar(5U);
    assert(tabela != NULL);
    assert(tabela_hash_tamanho(tabela) == 0U);
    assert(tabela_hash_fator_carga(tabela) == 0.0);
    tabela_hash_destruir(tabela);
    PASS("test_criar_e_destruir");
}

static void test_inserir_buscar_atualizar(void) {
    TabelaHash *tabela = tabela_hash_criar(3U);
    int valor = 0;
    assert(tabela != NULL);
...
static void test_remover(void) {
    TabelaHash *tabela = tabela_hash_criar(2U);
    int valor = 0;
    assert(tabela != NULL);
    assert(tabela_hash_inserir(tabela, 1, 10) == 1);
...
static void test_invalidos(void) {
    int valor = 0;
    assert(tabela_hash_criar(0U) == NULL);
